...
public:
  ...
  explicit GPUTexture(wgpu::Texture instance, std::string label)
    ...
    _memoryPressure = _computeMemoryPressure();
  }

  explicit GPUTexture(wgpu::Texture instance, std::string label, size_t memoryPressure)
          : NativeObject(CLASS_NAME), _instance(instance), _label(label),
...
public:
  ...
  size_t getMemoryPressure() override { return _memoryPressure; }

private:
  size_t _computeMemoryPressure();
  size_t _memoryPressure;
